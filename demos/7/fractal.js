var fractal = function(context){
	...
	function startGenerate(centerFuncCallback) {
		centerFunc = centerFuncCallback;
		drawCount = 0;
	...
	function setCircles() {
		...
		for (i = 0; i < numCircles; i++) {
			...
			var newCircle = {
				centerX: -maxR,
				centerY: context.canvas.height/2-50,
				maxRad : maxR,
				minRad : minR,
	...
	function onTimer() {
		...
		for (j = 0; j < drawsPerFrame; j++) {
			...
			for (i = 0; i < numCircles; i++) {
				...
				var center = {x: c.centerX, y: c.centerY};
				if(centerFunc) {
					center = centerFunc();
					if(center === null) {
						return stopGenerate();
				...
				context.setTransform(xSqueeze,0,0,1,center.x,center.y+yOffset)
